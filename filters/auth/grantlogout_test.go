...
func newGrantCookieWithTokens(config auth.OAuthConfig, refreshToken string, accessToken string) (*http.Cookie, error) {
	token := &oauth2.Token{
...
func TestGrantLogout(t *testing.T) {
	...
	t.Run("check that logout with both tokens revokes refresh token", func(t *testing.T) {
		...
		cookie, err := newGrantCookieWithTokens(*config, testRefreshToken, testToken)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("check that logout with no refresh token revokes access token", func(t *testing.T) {
		t.Log("make a logout request")
		cookie, err := newGrantCookieWithTokens(*config, "", testToken)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("check that logout with no tokens results in a 401", func(t *testing.T) {
		t.Log("make a logout request")
		cookie, err := newGrantCookieWithTokens(*config, "", "")
		if err != nil {
			t.Fatal(err)
		}
